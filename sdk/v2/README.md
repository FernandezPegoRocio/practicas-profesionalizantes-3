...
GET   
--> Devuelve la vista principal (HTML) en la ruta 
...
POST   
--> En la ruta /login autentica contra la BD y devuelve un token de sesión
...
--- src/login.js fue reescrito para consultar la base de datos con parámetros preparados en lugar de comparar contra credenciales hardcodeadas.
...
--- En main.mjs se importa auth.js 
El handler de /login genera un token al autenticar correctamente y lo devuelve en la respuesta.
hay otras dos rutas, que son: /logout y /checkAccess.

El CREATE TABLE de connect_db fue extendido para crear las tablas group, members, endpoint y access si no existen, respetando el modelo de datos trabajado en clase.
